...
if (webElement.tagName === 'DIV' && webElement.className && webElement.className.indexOf('nx-skootable-buttonicon') >= 0) {
   ...
   var inlineEditIcon = "ancestor::div[contains(@class,'slds-form-element__control')]//button[contains(@class,'test-id__inline-edit-trigger')]";
   var inlineEdit = document.evaluate(inlineEditIcon, webElement, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null);
   var inlineEditElem = inlineEdit.singleNodeValue;
   if (inlineEditElem) {
	   inlineEditElem.click();
	   return;
   }  
